...
class AuthService:
    ...
    async def register_user(data, db: AsyncSession) -> tuple[str, str]:
        """
            ...
            Raises:
                BadRequestException: Se o email já estiver em uso.
        """
        ...
        if existing:
            raise BadRequestException(message="Email já está em uso.")
    ...
    async def login_user(data, db: AsyncSession) -> tuple[str, str]:
        """
            ...
            Raises:
                UnauthorizedException: Se o email ou senha estiverem incorretos.
        """
        query = select(User).where(User.email == data.email)
        result = await db.execute(query)
        ...
        if not user or not verify_password(data.password, user.password_hash):
            raise UnauthorizedException(message="Email ou senha incorretos.")
    ...
    def refresh_token(refresh_token: str) -> tuple[str, str]:
        """
            ...
            Raises:
                UnauthorizedException: Se o refresh token for inválido ou expirado.
        """
        try:
            payload = jwt.decode(
                refresh_token,
            ...
            raise UnauthorizedException(message="Refresh token inválido.")
